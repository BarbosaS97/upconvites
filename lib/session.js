...
const DURACAO_MS = 1000 * 60 * 60 * 12; // 12 horas — só dura enquanto você está usando o painel
...
function definirCookieLogin(res) {
  ...
  // sem Max-Age: é um cookie de sessão, some quando o navegador fecha (a senha
  // continua sendo pedida de novo a cada vez que /admin é aberto do zero).
  res.setHeader('Set-Cookie', `${NOME_COOKIE}=${encodeURIComponent(valor)}; Path=/; HttpOnly${seguro}; SameSite=Lax`);
}
